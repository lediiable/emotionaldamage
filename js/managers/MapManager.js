    ...
    createIsometricMap(mapSize, cellSize) {
        for (let y = 0; y < mapSize; y++) {
            for (let x = 0; x < mapSize; x++) {
                ...
                // pour que ça apparaisse comme un carré véritable
...
// 4. Ajoutez la méthode pour limiter les déplacements de la caméra
    ...
    isValidBuildingLocation(gridX, gridY, size) {
        // Vérifier si l'emplacement est dans la zone débloquée
        if (!this.isInUnlockedRegion(gridX, gridY)) {
            return false;
        }

        // Vérifier s'il y a une superposition avec d'autres bâtiments
    ...
    checkBuildingOverlap(x1, y1, size1, building) {
        // Vérifier si les zones occupées par les deux bâtiments se chevauchent
        const x2 = building.x;
        const y2 = building.y;
        const size2 = building.size;

        // Considérer la taille des bâtiments pour la détection de collision
        return (
            x1 < x2 + size2 &&
            x1 + size1 > x2 &&
            y1 < y2 + size2 &&
    ...
    screenToGrid(screenX, screenY) {
        if (this.config.isIsometric) {
            ...
            // Convertir les coordonnées mondiales en coordonnées isométriques
            const isoX = (2 * worldY + worldX) / (this.config.cellSize);
            const isoY = (2 * worldY - worldX) / (this.config.cellSize);
